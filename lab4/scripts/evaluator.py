...
class EvalClassifier:
    ...
    def get_confusion_matrix(self, y_true, y_pred, classes):
        # Ensure labels in y_true and y_pred are within expected classes
        class_idx = {label: index for index, label in enumerate(classes)}
        cm = np.zeros((len(classes), len(classes)), dtype=int)

        for true_label, pred_label in zip(y_true, y_pred):
            # Skip any labels not in classes to prevent KeyError
    ...
    def plot_confusion_matrix(self, cm, classes):
        ...
        sbn.heatmap(data=cm, annot=True, cmap="Blues", fmt='d', cbar=True)
